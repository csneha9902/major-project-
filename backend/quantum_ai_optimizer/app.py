...
try:
    from quantum_ai_optimizer.auth import get_oauth_router, get_current_user
    AUTH_AVAILABLE = True
...
# Upload imports
from quantum_ai_optimizer.upload import process_edf_file
try:
    from quantum_ai_optimizer.upload import process_csv_file
    CSV_AVAILABLE = True
except Exception:
    CSV_AVAILABLE = False

# Analysis imports
from quantum_ai_optimizer.analysis import analyze_uploaded_data

# Export imports
from quantum_ai_optimizer.export import generate_pdf_report
...
try:
    ...
    def _create_demo_token():
        ...
        try:
            from quantum_ai_optimizer.auth.jwt_utils import create_access_token
...
# Include SNN Router
try:
    from quantum_ai_optimizer.snn.api import router as snn_router
...
async def get_metrics():
    ...
    h = read_json(Path("results/hybrid/metrics.json"))
    if b: out["baseline"] = b
    if h: out["hybrid"] = h
    ...
    if latest:
        ...
        out.setdefault("hybrid", latest.get("hybrid"))

    if not out.get("baseline") and not out.get("hybrid"):
...
from quantum_ai_optimizer.pipeline.baseline import train_baseline
from quantum_ai_optimizer.pipeline.hybrid import hybrid_run
from quantum_ai_optimizer.utils.wellness_tips import get_random_tip
from quantum_ai_optimizer.utils.session_tracker import SessionTracker
...
async def run_pipeline():
    ...
    await loop.run_in_executor(None, hybrid_run)
    await broadcast("Hybrid training done ✅")
    log("Hybrid training done")

    await broadcast("✅ Pipeline complete")
    log("✅ Pipeline complete")

    return {"status": "Pipeline complete", "log_file": str(log_file)}
from quantum_ai_optimizer.feedback import generate_feedback
...
async def get_history():
    ...
    if not p.exists():
        ...
        return {"runs": [
            {"ts": "T-2", "baseline": {"accuracy": 0.70, "auc": 0.72}, "hybrid": {"accuracy": 0.60, "auc": 0.62}},
            {"ts": "T-1", "baseline": {"accuracy": 0.78, "auc": 0.80}, "hybrid": {"accuracy": 0.65, "auc": 0.67}},
            {"ts": "T-0", "baseline": {"accuracy": 0.85, "auc": 0.90}, "hybrid": {"accuracy": 0.48, "auc": 0.49}},
        ]}
    try:
        ...
        for snap in arr:
            ...
            if "hybrid"   in snap: row["hybrid"]   = snap["hybrid"]
...
from quantum_ai_optimizer.streaming import DataStreamer
...
async def get_current_user_optional(credentials: HTTPAuthorizationCredentials = Depends(_optional_security)) -> dict:
    ...
    if credentials:
        try:
            if AUTH_AVAILABLE:
                from quantum_ai_optimizer.auth.jwt_utils import verify_token
